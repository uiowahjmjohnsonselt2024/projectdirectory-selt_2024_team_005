...
describe InventoryController, type: :controller do
  ...
  describe 'POST #use_item' do
    context 'when the item is a Potion' do
      before do
        fake_character.update(current_hp: 100)
        post :use_item, params: { index: 2 }, format: :json
      end
    ...
    context 'when the item is a Weapon' do
      let(:prev_weapon) { character.weapon_item_id }

      before do
        post :use_item, params: { index: 0 }, format: :json
      end

      it 'equips the weapon to the character' do
        character.reload
        expect(character.weapon_item_id).to eq(weapon.item_id)
      end

      it 'replaces the weapon in the inventory with the previously equipped weapon' do
        inventory.reload
        expect(inventory.items[1]).to eq(prev_weapon)
      end
    end

    context 'when the item is an Armor' do
      let(:prev_armor) { character.armor_item_id }

      before do
        post :use_item, params: { index: 1 }, format: :json
      end

      it 'equips the armor to the character' do
        character.reload
        expect(character.armor_item_id).to eq(armor.item_id)
      end

      it 'replaces the armor in the inventory with the previously equipped armor' do
        inventory.reload
        expect(inventory.items[1]).to eq(prev_armor)
      end
    end
  end
  describe 'POST #discard_item' do
    it 'should remove the item from the character\s inventory' do
      post :discard_item, params: { index: 0 }, format: :json
      inventory.reload
      expect(inventory.items[0]).to eq(nil)
    end
  end
end